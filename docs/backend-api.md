...
Актуально для Backend 0.2. Это описание реализованного API. Черновик участника 2
...
| --- | --- | --- |
| GET | `/health` | Готовность сервера, число товаров, настроен ли AI; корзина пока отключена |
...
Изменяющих корзину маршрутов пока нет. Запрос на покупку возвращает
...
Участник 3 реализует отдельный адаптер и серверный протокол подтверждения. Для него понадобится:
...
| Черновик Frontend | Реализованный Backend 0.2 |
| --- | --- |
| `text` | `message` |
...
| Cookie-сессия | Пока Bearer-токен из `/api/chat/sessions`; хранить в памяти UI |
| `error.code/message/retryable` | Реализовано |
| `/api/cart/*` | Пока не реализовано; совместная работа участников 1 и 3 |
