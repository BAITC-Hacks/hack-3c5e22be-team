...
Итоговая сборка использует один origin для сайта и API: `http://127.0.0.1:5173` при run_demo.py, `http://127.0.0.1:8000` при run_server.py. На внешнем стенде — относительные URL своего HTTPS origin. Полная схема: `/openapi.json`,
интерактивная документация: `/docs`. При включённой корзине повторное создание сессии с действующей HttpOnly cookie продолжает её без продления TTL; новый диалог сначала удаляет текущую сессию. Разные браузерные профили изолированы.
