...
1. `POST /api/chat/sessions` → `session_token`, `expires_in`. При включённой корзине и действующей cookie продолжается та же сессия браузера; иначе создаётся новая. TTL при продолжении не продлевается.
...
3. При создании сессии и успешном подтверждении устанавливается HttpOnly cookie
   `ekt_demo_session`, SameSite=Lax, Path=/. Токены не включаются в URL.
...
5. Для браузерного frontend нужны `credentials: "include"`, разрешённый CORS origin и одинаковое
   имя хоста. В итоговой сборке frontend и API имеют один origin, URL API относительные.
...
Cookie общая для вкладок одного хоста: вкладки продолжают одну действующую сессию. Явный «Новый диалог» удаляет сессию и cookie; другие вкладки получают SESSION_EXPIRED. Два разных профиля изолированы.
...
В итоговой сборке обработчики корзины подключены: credentials: include, количество, отдельное подтверждение и новые cart_action. Поля ответа чата сохранены. Корзина и frontend обслуживаются одним FastAPI origin.
