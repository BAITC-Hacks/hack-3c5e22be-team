...
const labels = { current: 'Номинальный ток', voltage: 'Напряжение', poles: 'Полюса', breaking_capacity: 'Отключающая способность', mounting: 'Установка', brand: 'Бренд' };
...
async function perform(action) {
  ...
  try { await action(); retryAction = null; }
    ...
    retryAction = error.status === 401 ? null : action;
...
async function send(text) {
  ...
  const action = async () => {
    ...
    node.append(el('div', 'mode', response.mode === 'rules' ? 'Базовый режим · поиск и ответы по данным каталога' : 'ИИ-помощник · факты из каталога'));
