...
def test_live_unknown_stale_conflicting_data_rejected(cart_client, raw_products):
    ...
    assert propose(c, auth).json()["error"]["code"] == "LIVE_CART_UNAVAILABLE"
    c.app.state.catalog.upsert(raw, datetime.now(UTC) - timedelta(days=1), "synthetic")
...
def test_stale_at_confirmation_and_no_client_price(cart_client, raw_products):
    ...
    proposal = propose(c, auth).json()
    c.app.state.catalog.upsert(raw_products[1], datetime.now(UTC) - timedelta(days=1), "synthetic")
